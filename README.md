...
Newspeak IDE is delivered via the web on the above link as a Progressive Web App (PWA). 
...
You could stop here and consult the Newspeak resources in the link above.

But you may decide to build and serve Newspeak or your own Newspeak application locally. 
The sections below are a guide for that.

Note: below, the terms related to 'serve', 'server' refers to 
      'serve in a HTTP server', 'HTTP server' or similar.

# 2. Newspeak application, its .vfuel file, how to build it, deploy it, and serve it
...
To serve a Newspeak application in a HTTP server, you need a binary file with `.vfuel` extension, one or more HTML files, and at least two files, 'primordialsoup.js' and 'primordialsoup.wasm' (both created by the `build.sh`), and potentially other application specific resources.
...
Unless you are targetting Fuchsia, follow the steps 1 to 3 in this section to install and build Primordialsoup and its dependencies.
...
2. **Install and configure the Emscripten SDK (`emsdk`):** Emscripten is a compiler that compiles C and C++ sources to WebAssembly, mainly for execution by browsers. 
...
Currently, only the Newspeak IDE application binary `HopscotchWebIDE.vfuel` can be deployed and served as a PWA out of the box (any other `.vfuel` application can be deployed as a website, see later items).
