...
const STEPS = [
  ["w1", "platform js global at: 'w1' put: 'basic'", "normal"],
  ["w2", "platform js global at: 'w2' put: 'plain-second'", "normal"],
  ["m1", "platform js global at: 'm1' put: ([:mb | platform js global at: 'mStart' put: 'STARTED'. mb nestedClasses addFromSource: 'public class ZY1 contents: c <String> = Document named: #ZY1 contents: c ()()'. platform js global at: 'mid' put: 'ADD-COMPLETED'. 'DONE'] value: ide documents documentHolderMixinBuilder)", "reselect"],
];
...
async function main() {
  ...
  ws.on('message',raw=>{const m=JSON.parse(raw); if(m.id&&pending.has(m.id)){pending.get(m.id)(m.result);pending.delete(m.id);return;}
    ...
    else if(m.method==='Runtime.consoleAPICalled'){const a=m.params.args.map(x=>x.value!==undefined?String(x.value):(x.description||x.type)).join(' ');log.push('['+m.params.type+'] '+a);}});
  ...
  await ev(`(function(){
    window._sels=[];window._errs=[];
    window.addEventListener('error',function(e){window._errs.push('onerror: '+e.message)});
    window.addEventListener('unhandledrejection',function(e){window._errs.push('unhandledrejection: '+String(e.reason).slice(0,200))});
    document.querySelectorAll('.CodeMirror').forEach(function(el,i){
      var c=el.CodeMirror; if(!c||c._selWrapped)return; c._selWrapped=true;
      var g=c.getSelection.bind(c);
      c.getSelection=function(){var r=g.apply(null,arguments);window._sels.push([i,JSON.stringify(r).slice(0,60)]);return r};
    });
    return 'SPIED '+document.querySelectorAll('.CodeMirror').length;
  })()`).then(r=>console.log('spy install:',r));
  for (const [flag, expr, mode] of STEPS) {
    await ev(`(function(){
      document.querySelectorAll('.CodeMirror').forEach(function(el,i){
        var c=el.CodeMirror; if(!c||c._selWrapped)return; c._selWrapped=true;
        var g=c.getSelection.bind(c);
    ...
    await new Promise(r=>setTimeout(r, mode === 'reselect' ? 6000 : 2500));
    if (mode === 'reselect') {
      // Echoes of setValue's cursor events have drained; re-select and click
      // in one breath so nothing can wipe the selection before dispatch.
      await ev(`(function(){var cms=document.querySelectorAll('.CodeMirror');var cm=cms[cms.length-1].CodeMirror;cm.focus();cm.execCommand('selectAll');var m=Array.from(document.querySelectorAll('a,button,span,div,label')).filter(e=>(e.innerText||'').trim()==='Evaluate Selection');var e=m[m.length-1];['mousedown','mouseup','click'].forEach(t=>e.dispatchEvent(new MouseEvent(t,{bubbles:true,cancelable:true,view:window})));return 'RC'})()`);
    } else {
  ...
  console.log('getSelection calls:', JSON.stringify(await ev('window._sels')));
  console.log('js errors:', JSON.stringify(await ev('window._errs')));
  console.log('CM count:', await ev("document.querySelectorAll('.CodeMirror').length"));
  console.log('subscription keys:', JSON.stringify(await ev("Array.from(newspeakSubscriptions.keys()).filter(k=>k.indexOf('button')>=0||k.indexOf('codemirror')>=0)")));
